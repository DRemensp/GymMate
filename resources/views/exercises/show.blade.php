    ...
    <div class="min-h-screen px-6 pt-8 pb-10">
        <div class="max-w-2xl mx-auto">
            ...
            <div class="sm:pl-14 space-y-6"
                ...
                <div class="bg-zinc-900/60 backdrop-blur-sm border border-zinc-800 rounded-2xl overflow-hidden">
                                ...
                                @foreach($sessions as $session)
                                    @foreach($session->sets as $set)
                                        <tr x-on:click="Livewire.dispatchTo('edit-workout-session', 'load-session', { id: {{ $session->id }} })"
                                            ...
                                            {{-- Datum nur beim ersten Set der Session zeigen --}}
                                            <td class="px-5 py-2.5 text-zinc-400">
                                                @if($loop->first)
                                                    <span class="group-hover:text-orange-400 transition-colors">{{ $session->logged_at->format('d.m.Y') }}</span>
                                                    <span class="block text-zinc-600 text-xs">{{ $session->logged_at->format('H:i') }}</span>
                                                @endif
                                            </td>
                                            <td class="px-3 py-2.5 text-zinc-500 font-mono text-xs">{{ $set->set_number }}</td>
                                            <td class="px-3 py-2.5 text-white font-semibold">{{ $set->weight }} kg</td>
                                            <td class="px-3 py-2.5 text-zinc-300">{{ $set->reps }}x</td>
                                        </tr>
                                    @endforeach
                                @endforeach
                            </tbody>
                ...
                @else
                    <p class="text-zinc-600 text-sm text-center py-4">No sessions logged yet.</p>
                @endif
