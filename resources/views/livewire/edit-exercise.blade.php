<div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-md p-6 border border-zinc-300 dark:border-zinc-700">
            <h2 class="text-lg font-semibold text-zinc-900 dark:text-white mb-5">Übung bearbeiten</h2>
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <button type="button"
                        @click="window.OfflineQueue.isOffline().then(offline => {
                            if (offline) {
                                window.OfflineQueue.enqueue('update_exercise', {
                                    exercise_id:   $wire.exerciseId,
                                    name:          $wire.name,
                                    description:   $wire.description || null,
                                    is_unilateral: $wire.is_unilateral ? 1 : 0,
                                });
                                $wire.set('open', false);
                            } else {
                                $wire.save();
