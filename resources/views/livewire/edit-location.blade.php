<div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-md p-6 border border-zinc-300 dark:border-zinc-700">
            <h2 class="text-lg font-semibold text-zinc-900 dark:text-white mb-5">Standort bearbeiten</h2>

            <form wire:submit="save" class="space-y-4">
                <div>
                    <label class="block text-sm text-zinc-500 dark:text-zinc-400 mb-1">Name</label>
                    <input wire:model="name" type="text" placeholder="z.B. Fitnessstudio"
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <button type="button"
                        @click="window.OfflineQueue.isOffline().then(offline => {
                            if (offline) {
                                window.OfflineQueue.enqueue('update_location', {
                                    location_id: $wire.locationId,
                                    name:        $wire.name,
                                });
                                $wire.set('open', false);
                            } else {
