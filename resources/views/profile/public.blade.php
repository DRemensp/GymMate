    ...
    <div class="min-h-screen px-4 pt-6 pb-12">
        <div class="max-w-lg mx-auto space-y-4">
            ...
            <div class="relative bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[22px] overflow-hidden">
                <div class="absolute top-0 left-1/2 -translate-x-1/2 w-56 h-28 bg-orange-500/15 rounded-full blur-3xl pointer-events-none"></div>

                <div class="relative px-5 pt-7 pb-6">
                    @auth
                        @if(Auth::user()->name === $user->name)
                        <a href="{{ route('settings.edit') }}"
                            class="absolute top-4 right-4 flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-zinc-500 dark:text-zinc-400 bg-zinc-100 dark:bg-zinc-800 rounded-xl hover:text-orange-500 transition-colors">
                    ...
                    <div class="flex flex-col items-center text-center">
                        <div class="w-[72px] h-[72px] rounded-full overflow-hidden bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-[28px] font-bold text-white shadow-lg shadow-orange-500/30 mb-3 shrink-0">
                            @if($user->avatarUrl())
            ...
            <div class="grid grid-cols-2 gap-3">
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[18px] p-4">
                    ...
                    <div class="text-[28px] font-medium text-orange-500 tracking-tight leading-none">{{ number_format($totalSessions) }}</div>
                </div>
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[22px] overflow-hidden"
                ...
                <div class="flex gap-2 overflow-x-auto px-4 py-3 scrollbar-none">
                    ...
                    <button
                        ...
                        <span class="w-1.5 h-1.5 rounded-full
                            {{ $entry?->is_rest ? 'bg-zinc-300 dark:bg-zinc-600' : ($entry?->exercises->isNotEmpty() ? 'bg-orange-400' : 'bg-transparent') }}"
                ...
                <div x-show="selected === {{ $dow }}" x-cloak
                     class="border-t border-zinc-100 dark:border-zinc-800 {{ $dow === $todayDow ? 'border-orange-500/20' : '' }}">
                    <div class="flex items-center gap-2 px-5 py-3 border-b border-zinc-100 dark:border-zinc-800 {{ $dow === $todayDow ? 'bg-orange-500/5' : '' }}">
                        <span class="text-sm font-semibold {{ $dow === $todayDow ? 'text-orange-400' : 'text-zinc-700 dark:text-zinc-300' }}">
                            {{ $dayFull[$dow] }}
                        </span>
                        @if($dow === $todayDow)
